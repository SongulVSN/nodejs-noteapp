...
const updateNoteStatus = async (req, res) => {
  try {
    ...
    if (completed === undefined) {
      return res.status(400).send("Tamamlanma durumu belirtilmedi.");
    }

    const noteRef = db.ref(`notes/${noteId}`);
    await noteRef.update({ completed });
    res.send("Notun durumu güncellendi!");
  } catch (error) {
    res.status(500).send("Not durumu güncellenirken bir hata oluştu!");
  }
};
